...
def percentage_correct_answers(player):
    ...
    try:
        ...
        percentage_correct = int(float(format((correct_answers/count_player_answers), ".2f"))*100)
...
def quizes_data(request):
    ...
    if request.user.is_authenticated:
        number_quizes_played = PlayedQuiz.objects.filter(player=request.user).count()
        ...
        leaderboard = sorted(score_list, key=lambda k: k["score"], reverse=True)
